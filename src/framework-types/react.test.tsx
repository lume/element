...
describe('JSX types with ReactElementAttributes', () => {
	it('derives JSX types from classes', () => {
		;<>
			...
			{/* @ts-expect-error good, event handler is not valid (although at runtime React will listen for "notanevent" when a function is passed) */}
			<some-element onnotanevent={(e: Event) => e} />

			{/* @ts-expect-error Ensure that non-event-listener function on* properties are ignored, as React will always listen to events based on their type being a function which is incorrect (the functions will receive an event instead of the arguments they expect). (Use "as any" to ensure we check property existence, not value type.) */}
		...
		type props = ReactElementAttributes<ElementClass, 'num' | 'str' | 'bool' | 'onfoo' | 'oncLick'>
